...
function removeLastOccurrences(str, value) {
  ...
  const v = value.split('').reverse().join('');
  const st = s.replace(v, '');
...
function containsSubstring(str, substring) {
  return str.includes(str, substring);
}
...
function encodeToRot13(str) {
  const inText = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const outText = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[a-z]/gi, (letter) => outText[inText.indexOf(letter)]);
}
